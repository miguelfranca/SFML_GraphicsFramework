...
namespace GF {
	Slider<GF::Circle>::Slider(const float size, const sf::Vector2f pos, const std::string o, const double mmax, const double mmin)
	{
		orientacion = o;
		slider.setup(7, pos, sf::Color(150, 150, 150), sf::Color::White, 1);

		if (orientacion == HORIZONTAL) 
		{
			line = GF::Line(sf::Vector2f(pos.x - size / 2, pos.y), sf::Vector2f(pos.x + size / 2, pos.y));
			box = GF::Rectangle(sf::Vector2f(size, 40), pos, CENTERED);
		}
		if (orientacion == VERTICAL)
		{
			line = GF::Line(sf::Vector2f(pos.x, pos.y - size / 2), sf::Vector2f(pos.x, pos.y + size / 2));
			box = GF::Rectangle(sf::Vector2f(40, size), pos, CENTERED);
		}
		max = mmax;
		min = mmin;
	}

	Slider<GF::Rectangle>::Slider(const sf::Vector2f size, const sf::Vector2f pos, const std::string o, const double mmax, const double mmin)
	{
		orientacion = o;
		max = mmax;
		min = mmin;
